...
def saveMask(mask,path):
    ...
    mask = cv2.cvtColor(np.uint8(mask * 255), cv2.COLOR_RGB2BGR)
    cv2.imwrite(path, mask)
    
def saveMask2(mask,path):
    mask=mask.cpu()
    mask=mask.detach().numpy()
    mask = cv2.cvtColor(np.uint8(mask * 255), cv2.COLOR_RGB2BGR)
    cv2.imwrite(path, mask)
...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None,
    ...
    for batch in pbar:
        ...
        # pdb.set_trace()
        if(count%1==0):
            # a=np.transpose(inp.squeeze().cpu().detach().numpy(), (1, 2,0))
            # a= cv2.cvtColor(a, cv2.COLOR_RGB2BGR)
            # cv2.imwrite("original/"+str(count)+".png",a)
            saveMask2(inp,"_NeuralTexturesresult2/"+str(count)+".png")
            saveMask(pred.squeeze(),"_NeuralTexturesresult2/"+str(count)+".png")
            saveMask(batch['gt'].squeeze(),"_NeuralTexturesGT/"+str(count)+".png")
